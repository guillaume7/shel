...
$$
\begin{aligned}
\frac{\partial Hu}{\partial t} &= - \frac{(H u u)_{i + 1/2} - (H u u)_{i - 1/2}}{\Delta x} \\
& - \frac{(H u v)_{j + 1/2} - (H u v)_{j - 1/2}}{\Delta y} \\
& + f(H v) \\
...
$$
\begin{aligned}
(H u u)_{i + 1/2} &= m_{U\,i+1}\,H(u_{i + 1} + u)^2 /4 \\
(H u u)_{i - 1/2} &= m_{U\,i-1}\,H_{i - 1} (u + u_{i - 1})^2 /4 \\
(H u v)_{j + 1/2} &= m_{U\,j+1}\,(H_{i - 1} + H_i + H_{i - 1,j + 1} + H_{i,j + 1}) \\
...
Notice how the $(H u u)_{j + 1/2}$, $(H u v)_{j - 1/2}$, $f(H v)$ and $C_D u\sqrt{u^2 + v^2}$ terms lose significant precision compared to the other terms, due to their 4-term averaging.
...
- switch signal of the Coriolis term: $(+ \leftrightarrow -)$
...
$$
\begin{aligned}
\frac{\partial \eta}{\partial t} &= - \frac{(H u)_{i + 1/2} - (H u)_{i - 1/2}}{\Delta x} \\
& - \frac{(H v)_{j + 1/2} - (H v)_{j - 1/2}}{\Delta y} \\
& \equiv R\eta
\end{aligned}
$$
...
$$
\begin{aligned}
(H u)_{i + 1/2} &= m_{T\,i+1}\,(H + H_{i + 1})/2\;u_{i + 1} \\
(H u)_{i - 1/2} &= m_{T\,i-1}\,(H_{i - 1} + H)/2\;u \\
(H v)_{j + 1/2} &= m_{T\,j+1}\,(H + H_{j + 1})/2\;v_{j + 1} \\
(H v)_{j - 1/2} &= m_{T\,j-1}\,(H_{j - 1} + H)/2\;v
\end{aligned}
$$
