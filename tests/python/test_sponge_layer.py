...
def test_sponge_eta_blending_west_linear():
    ...
    cfg = {
        "boundary_conditions": {"west": "flather", "east": "closed", "south": "closed", "north": "closed"},
        "boundary_eta_ext": {"west": eta_ext},
        "eta_bc_stage": "post",
        "boundary_eta_relax": 1.0,
        "sponge": {"enabled": True, "width": 3, "alpha": 0.5, "taper": "linear", "apply_to": "eta"},
    }

    eta1, U1, V1 = explicit_step_with_config(eta, H, U, V, dt=0.1, dx=1.0, dy=1.0, config=cfg)
...
def test_sponge_momentum_relaxes_toward_boundary_u_east():
    ...
    cfg = {
        ...
        "sponge": {"enabled": True, "width": 3, "alpha": 0.5, "taper": "cosine", "apply_to": "momentum"},
    }
