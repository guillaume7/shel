...
def test_flather_west_and_south_sides():
    ...
    eta_ext_w[:, 0] = 0.05
    bc.apply_side(U, V, "west", U_old=U_old, V_old=V_old, H=H, g=g, dt=1.0, dx=1.0, dy=1.0, eta_old=eta_old, eta_ext=eta_ext_w)
    ...
    eta_ext_s[0, :] = 0.02
    bc.apply_side(U, V, "south", U_old=U_old, V_old=V_old, H=H, g=g, dt=1.0, dx=1.0, dy=1.0, eta_old=eta_old, eta_ext=eta_ext_s)
    c = float(np.sqrt(g * np.mean(H[0, :])))
    Hm = float(np.maximum(1e-12, np.mean(H[0, :])))
