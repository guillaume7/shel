...
class MomentumTendency(Protocol):  # lightweight structural type
    def compute(
        self, u: NDArray, v: NDArray, *args, **kwargs
...
class BaseMomentumAdvection(ABC):
    @abstractmethod
    def compute(
        self, u: NDArray, v: NDArray, H: NDArray, grid, **kwargs
    ) -> Tuple[NDArray, NDArray]: ...
...
class BasePressureGradient(ABC):
    @abstractmethod
    def compute(
        self, eta: NDArray, H: NDArray, grid, gravity: float
    ) -> Tuple[NDArray, NDArray]: ...
...
class BaseViscosityOperator(ABC):
    @abstractmethod
    def compute(
        self, u: NDArray, v: NDArray, grid, nu: float
    ) -> Tuple[NDArray, NDArray]: ...
...
class BaseFriction(ABC):
    @abstractmethod
    def compute(
        self, u: NDArray, v: NDArray, H: NDArray, grid, **params
    ) -> Tuple[NDArray, NDArray]: ...
