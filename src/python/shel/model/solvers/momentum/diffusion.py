...
def _laplacian(arr: Array, dx: float, dy: float) -> Array:
    ...
    # interior points
    out[1:ny-1, 1:nx-1] = (
        (arr[1:ny-1, 2:] - 2 * arr[1:ny-1, 1:nx-1] + arr[1:ny-1, 0:nx-2]) / (dx * dx)
        + (arr[2:, 1:nx-1] - 2 * arr[1:ny-1, 1:nx-1] + arr[0:ny-2, 1:nx-1]) / (dy * dy)
    )
...
def viscous_tendency(U: Array, V: Array, nu: float, dx: float, dy: float) -> tuple[Array, Array]:
