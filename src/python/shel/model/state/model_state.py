...
from shel.model.grid import build_all_masks
...
class ModelState:
    ...
    def _initialize_fields(self) -> None:
        ...
        self.eta = np.zeros((ny, nx)); self.eta_old = self.eta.copy(); self.eta_new = self.eta.copy()
        self.H = np.zeros((ny, nx)); self.H_old = self.H.copy(); self.H_new = self.H.copy()
        self.u = np.zeros((ny, nx + 1)); self.u_old = self.u.copy(); self.u_new = self.u.copy()
        self.v = np.zeros((ny + 1, nx)); self.v_old = self.v.copy(); self.v_new = self.v.copy()
    ...
    def set_bathymetry(self, d: NDArray) -> None:
        ...
        self.H = self.eta + self.d; self.H_old = self.eta_old + self.d; self.H_new = self.eta_new + self.d
    ...
    def set_initial_elevation(self, eta: NDArray) -> None:
        ...
        self.eta = eta.copy(); self.eta_old = eta.copy(); self.eta_new = eta.copy()
        self.H = self.eta + self.d; self.H_old = self.eta_old + self.d; self.H_new = self.eta_new + self.d

    def set_initial_velocities(self, u: Optional[NDArray] = None, v: Optional[NDArray] = None) -> None:
        if u is not None:
            if u.shape != (self.grid.ny, self.grid.nx + 1):
                raise ValueError("U shape mismatch")
            self.u = u.copy(); self.u_old = u.copy(); self.u_new = u.copy()
        if v is not None:
            if v.shape != (self.grid.ny + 1, self.grid.nx):
                raise ValueError("V shape mismatch")
            self.v = v.copy(); self.v_old = v.copy(); self.v_new = v.copy()
    ...
    def compute_vorticity(self) -> NDArray: return FieldDiagnostics.vorticity(self.u, self.v, self.grid)
    def compute_kinetic_energy(self) -> float:
        u, v = self._masked_uv()
        return IntegratedDiagnostics.kinetic_energy(u, v, self.H, self.grid)
    def compute_potential_energy(self) -> float: return IntegratedDiagnostics.potential_energy(self.eta, self.grid, self.gravity)
    def compute_total_energy(self) -> float: return IntegratedDiagnostics.total_energy(self.u, self.v, self.eta, self.H, self.grid, self.gravity)
    def compute_volume(self) -> float: return IntegratedDiagnostics.volume(self.H, self.grid)
    def compute_okubo_weiss(self) -> NDArray: return FieldDiagnostics.okubo_weiss(self.u, self.v, self.grid)
    def compute_enstrophy(self) -> float: return IntegratedDiagnostics.enstrophy(self.u, self.v, self.grid)
    def compute_potential_enstrophy(self) -> float: return IntegratedDiagnostics.potential_enstrophy(self.u, self.v, self.H, self.coriolis, self.grid)

    # Time management
    def advance_time(self) -> None: self.time += self.timestep; self.step += 1
    def swap_time_levels(self) -> None:
        self.eta_old = self.eta.copy(); self.eta = self.eta_new.copy()
        self.H_old = self.H.copy(); self.H = self.H_new.copy()
        self.u_old = self.u.copy(); self.u = self.u_new.copy()
        self.v_old = self.v.copy(); self.v = self.v_new.copy()
    ...
    def _serialize_masks(self):
        ...
        return {"t": opt(getattr(self, "mask_t", None)), "u": opt(self.mask_u), "v": opt(self.mask_v), "q": opt(self.mask_q)}

    def to_dict(self) -> Dict[str, Any]:
        integ = IntegratedDiagnostics.as_dict(self.u, self.v, self.eta, self.H, self.grid, self.gravity, self.coriolis)
        fields = FieldDiagnostics.as_dict(self.u, self.v, self.grid)
        return {
            "time": self.time,
            "step": self.step,
            "grid": {"nx": self.grid.nx, "ny": self.grid.ny, "dx": self.grid.dx, "dy": self.grid.dy, "x_origin": self.grid.x_origin, "y_origin": self.grid.y_origin},
            "fields": {"eta": self.eta.tolist(), "u": self.u.tolist(), "v": self.v.tolist(), "d": self.d.tolist(), "H": self.H.tolist()},
            "masks": self._serialize_masks(),
            "parameters": {"gravity": self.gravity, "viscosity": self.viscosity, "bottom_drag_coef": self.bottom_drag_coef},
            "diagnostics": {"integrated": integ, "fields": {k: v.tolist() for k, v in fields.items()}},
        }
