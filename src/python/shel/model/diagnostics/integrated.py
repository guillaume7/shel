...
class IntegratedDiagnostics:
    ...
    # ------------------------------ energies ------------------------------
    @staticmethod
    def kinetic_energy(u: NDArray, v: NDArray, H: NDArray, grid: Grid, rho: float = RHO0) -> float:
    ...
    @staticmethod
    def potential_energy(eta: NDArray, grid: Grid, gravity: float, rho: float = RHO0) -> float:
        area_t, _, _ = grid.compute_areas()
        return float(0.5 * rho * gravity * np.sum(eta**2 * area_t))

    @staticmethod
    def total_energy(u: NDArray, v: NDArray, eta: NDArray, H: NDArray, grid: Grid, gravity: float, rho: float = RHO0) -> float:
        return IntegratedDiagnostics.kinetic_energy(u, v, H, grid, rho) + IntegratedDiagnostics.potential_energy(
            eta, grid, gravity, rho
        )
    ...
    @staticmethod
    def potential_enstrophy(u: NDArray, v: NDArray, H: NDArray, f: NDArray, grid: Grid) -> float:
    ...
    # ------------------------------ collections ------------------------------
    @staticmethod
    def energy_partition(u: NDArray, v: NDArray, eta: NDArray, H: NDArray, grid: Grid, gravity: float, rho: float = RHO0) -> Tuple[float, float, float]:
        ke = IntegratedDiagnostics.kinetic_energy(u, v, H, grid, rho)
        pe = IntegratedDiagnostics.potential_energy(eta, grid, gravity, rho)
        return ke, pe, ke + pe

    @staticmethod
    def as_dict(u: NDArray, v: NDArray, eta: NDArray, H: NDArray, grid: Grid, gravity: float, coriolis: NDArray) -> Dict[str, float]:
        ...
        return {
            ...
            "potential_enstrophy": IntegratedDiagnostics.potential_enstrophy(u, v, H, coriolis, grid),
        }
